...
def test_init():
    ...
    assert min_max_round(DATA_Z_STAG[:, 0, :, :]) == (0.0, 0.0)
    assert min_max_round(DATA_Z_STAG[:, 1, :, :]) == (55.5, 60.5)  # Partially contains 60
    assert min_max_round(DATA_Z_STAG[:, 2, :, :]) == (119.2, 129.9)
    assert min_max_round(DATA_Z_STAG[:, 3, :, :]) == (247.3, 269.8)  # Fully contains 350
    assert min_max_round(DATA_Z_STAG[:, 4, :, :]) == (376.7, 411.1)
